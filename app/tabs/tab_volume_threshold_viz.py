...
threshold band, so roads with similar traffic volume profiles share a threshold.

Traces per cluster (all individually toggleable via the Plotly legend):
  • Actual volume observations  — scatter, colour-coded by cluster
...
def render(ctx: AppContext) -> None:
    ...
    st.caption(
        "Volume observations colour-coded by K-Means cluster.  "
        "P10 (solid) and P90 (dashed) threshold lines are derived **per cluster** — "
        "each cluster's own volume distribution determines its threshold, "
        "so thresholds adapt to the road's traffic volume profile."
    )
    ...
    avail = discover_cluster_results(ctx, "volume")
    if not avail:
        st.info(
            "No volume clustering results found on disk or in session state.  "
            "Go to the **Clustering** tab and run a **volume** clustering first."
        )
        return

    sel_stem  = st.selectbox(
        "Select volume clustering result", list(avail.keys()), key="vol_thresh_sel_stem"
    )
    ...
    show_points  = t1.checkbox("Scatter points",     value=True,  key="vol_thresh_t_pts")
    show_p10     = t2.checkbox("P10 threshold",      value=True,  key="vol_thresh_t_p10")
    show_p90     = t3.checkbox("P90 upper band",     value=True,  key="vol_thresh_t_p90")
    show_mean    = t4.checkbox("Hourly mean",        value=False, key="vol_thresh_t_mean")
    show_medoids = t5.checkbox("Medoid markers",     value=True,  key="vol_thresh_t_med")
    ...
    if not volume_col or not hour_col:
        st.error(
            f"Could not detect **VOLUME** or **hour** columns.  "
    ...
    df = df.dropna(subset=[hour_col, volume_col, cluster_col])
    ...
    band_cache_key = ("volume", cache_key, day_sel)
    if band_cache_key not in bands_cache:
        bands_cache[band_cache_key] = compute_cluster_bands(
            df_thresh, volume_col, hour_col, cluster_col, include_hourly=False
        )[0]
    ...
    if show_medoids:
        ...
        if medoid_cache_key not in medoid_cache:
            medoid_cache[medoid_cache_key] = compute_medoids(
                df_filt, volume_col, hour_col, cluster_col
            )
        ...
        for cid in cluster_ids:
            ...
            fig.add_trace(go.Scatter(
                ...
                hovertemplate=(
                    ...
                    "Volume: %{y:.0f} veh/hr<extra></extra>"
                ),
            ))
    ...
    if show_p10:
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=(
                "Hour: %{x}<br>"
                "<b>P10 threshold: %{y:.0f} veh/hr</b><br>"
    ...
    if show_mean:
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=(
                "Hour: %{x}<br>Mean: %{y:.0f} veh/hr<extra></extra>"
            ),
        ))
        ...
        for _, row in medoids_df.iterrows():
            ...
            fig.add_trace(go.Scatter(
                ...
                hovertemplate=(
                    ...
                    f"Volume: {row[volume_col]:.0f} veh/hr<extra></extra>"
                ),
            ))

    # ── Layout ────────────────────────────────────────────────────────────────
    title = f"Volume vs Hour of Day — {sel_stem}"
    if day_sel != "All":
        title += f"  [{day_sel}]"
    if road_sel:
    ...
    band_lookup = overall_bands.set_index("cluster_label")[["p10", "p90"]].rename(
        columns={"p10": "cluster_p10_veh_hr", "p90": "cluster_p90_veh_hr"}
    )
    df_out = df_filt.copy()
    df_out = df_out.join(band_lookup, on=cluster_col)
    export_cols = [
        c for c in df_filt.columns if c != cluster_col
    ] + [cluster_col, "cluster_p10_veh_hr", "cluster_p90_veh_hr"]
    ...
    with dl1:
        st.download_button(
            ...
            file_name="volume_data_with_cluster_thresholds.csv",
            mime="text/csv",
            help=(
                "Filtered rows + cluster_label + cluster_p10_veh_hr + "
                "cluster_p90_veh_hr."
            ),
        )
    with dl2:
        display_bands_dl = overall_bands.rename(columns={
            "cluster_label": "Cluster",
            "p10": "P10 threshold (veh/hr)", "p90": "P90 upper band (veh/hr)",
            "mean": "Mean volume (veh/hr)", "size": "Records",
        })
        st.download_button(
            "⬇  Download cluster band summary (CSV)",
            data=display_bands_dl.to_csv(index=False),
            file_name="volume_cluster_bands.csv",
            mime="text/csv",
