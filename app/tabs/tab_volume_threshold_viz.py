"""
Tab: Volume Threshold Visualisation
Volume (veh/hr) on the Y-axis, Hour of day (0-23) on the X-axis.

Traces (each individually toggleable via the Plotly legend):
  • Actual volume observations  — scatter points
  • P10 adaptive threshold      — per-hour 10th-percentile line
  • P90 upper band              — per-hour 90th-percentile line  (optional)
  • Hourly mean volume          — per-hour mean line             (optional)

Cascading filters: ROAD_NAME → LINK_ID → Day type.
"""
...
# ── colour palette ────────────────────────────────────────────────────────────
_COL_POINTS = "rgba(100, 160, 230, 0.35)"
_COL_P10    = "#E53935"
_COL_P90    = "#43A047"
_COL_MEAN   = "#FB8C00"
    ...
    with col_sel:
        if preproc_files:
            sel = st.selectbox(
                ...
                key="vol_thresh_preproc_sel",
            )
            if sel != "(none)":
                ppath = preproc_files[sel]
                if (
                    st.session_state.get("_vol_thresh_loaded") != ppath
                    or st.session_state.get("_vol_thresh_df") is None
                ):
                    st.session_state["_vol_thresh_df"]     = pd.read_csv(ppath)
                    st.session_state["_vol_thresh_loaded"] = ppath
                st.success(f"Loaded {len(st.session_state['_vol_thresh_df']):,} rows")
        else:
            st.info("No preprocessed files found on disk.")

    with col_up:
        uploaded = st.file_uploader(
            "Or upload any CSV", type=["csv"], key="vol_thresh_upload"
        )
        if uploaded is not None:
            try:
                df_up = pd.read_csv(uploaded)
                st.session_state["_vol_thresh_df"]     = df_up
                st.session_state["_vol_thresh_loaded"] = None
                st.success(f"Loaded {len(df_up):,} rows  ·  {len(df_up.columns)} columns")
            except Exception as e:
                st.error(f"Could not read file: {e}")

    return st.session_state.get("_vol_thresh_df")
...
def _detect_cols(df: pd.DataFrame, ctx: AppContext) -> dict:
    ...
    return {
        "volume":    dc.get("volume") or col_lower.get("volume"),
...
def render(ctx: AppContext) -> None:
    st.header("Volume Threshold Visualisation")
    st.caption(
        "Volume observations plotted against hour of day.  "
    ...
    if not volume_col or not hour_col:
        st.error(
            "Could not detect **VOLUME** or **hour** columns in this file. "
    ...
    df = df.dropna(subset=[hour_col, volume_col])
    ...
    with fc1:
        ...
        day_sel = st.selectbox("Day type", day_opts, key="vol_thresh_day")

    with fc3:
        scatter_sample = st.number_input(
            "Max scatter points", min_value=500, max_value=100_000,
            value=10_000, step=500, key="vol_thresh_scatter_n",
        )
    ...
    if has_road_col:
        ...
        prev_road = st.session_state.get("_vol_thresh_prev_road", [])
        road_sel  = st.multiselect(
            f"Filter by {road_name_col} (leave empty = all)",
            all_roads, default=[], key="vol_thresh_road_names",
        )
        if road_sel != prev_road:
            st.session_state["_vol_thresh_prev_road"] = road_sel
            st.session_state.pop("vol_thresh_ids", None)
    ...
    if has_id_col:
        ...
        id_sel = st.multiselect(
            ...
            id_pool, default=id_default, key="vol_thresh_ids",
        )
        if caption:
            st.caption(caption)
    ...
    show_points = t1.checkbox("Actual volume points",    value=True,  key="vol_thresh_t_pts")
    show_p10    = t2.checkbox("P10 adaptive threshold",  value=True,  key="vol_thresh_t_p10")
    show_p90    = t3.checkbox("P90 upper band",          value=False, key="vol_thresh_t_p90")
    show_mean   = t4.checkbox("Hourly mean",             value=False, key="vol_thresh_t_mean")

    # ── Hourly aggregates ─────────────────────────────────────────────────────
    hourly = (
        df_filt.groupby(hour_col)[volume_col]
        .agg(
    ...
    if show_points:
        df_scatter = df_filt.sample(min(int(scatter_sample), len(df_filt)), random_state=42)
        jitter = np.random.default_rng(0).uniform(-0.35, 0.35, len(df_scatter))
        fig.add_trace(go.Scatter(
            x=df_scatter[hour_col].values + jitter,
            y=df_scatter[volume_col].values,
            mode="markers",
            name="Actual volume",
            marker=dict(color=_COL_POINTS, size=4, line=dict(width=0)),
            hovertemplate="Hour: %{x:.1f}<br>Volume: %{y:.0f} veh/hr<extra></extra>",
        ))

    if show_p90:
        fig.add_trace(go.Scatter(
            x=hours, y=p90_vals, mode="lines+markers",
            name="P90 upper band",
            line=dict(color=_COL_P90, width=2.5, dash="dot"),
            marker=dict(size=6),
            hovertemplate="Hour: %{x}<br>P90: %{y:.0f} veh/hr<br>n=%{customdata:,}<extra></extra>",
            customdata=count_vals,
        ))

    if show_mean:
        fig.add_trace(go.Scatter(
            x=hours, y=mean_vals, mode="lines+markers",
            name="Hourly mean",
            line=dict(color=_COL_MEAN, width=2.5, dash="dash"),
            marker=dict(size=6),
            hovertemplate="Hour: %{x}<br>Mean: %{y:.0f} veh/hr<br>n=%{customdata:,}<extra></extra>",
            customdata=count_vals,
        ))

    if show_p10:
        fig.add_trace(go.Scatter(
            x=hours, y=p10_vals, mode="lines+markers",
            name="P10 adaptive threshold",
            line=dict(color=_COL_P10, width=3),
            marker=dict(size=7, symbol="circle"),
            hovertemplate=(
                "Hour: %{x}<br><b>P10 threshold: %{y:.0f} veh/hr</b>"
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="Volume (veh/hr)", gridcolor="#EEEEEE", rangemode="tozero"),
    ...
    hour_lookup = hourly.set_index(hour_col)[["p10", "p90", "mean"]].rename(
        columns={
            "p10":  "p10_threshold_veh_hr",
            "p90":  "p90_upper_band_veh_hr",
            "mean": "hourly_mean_veh_hr",
        }
    )
    df_enriched = df_filt.copy()
    df_enriched = df_enriched.join(hour_lookup, on=hour_col)
    df_enriched["below_p10_threshold"] = (
        df_enriched[volume_col] < df_enriched["p10_threshold_veh_hr"]
    )

    display_hourly = hourly.rename(columns={
        hour_col: "Hour",
        "p10":    "P10 threshold (veh/hr)",
        "p90":    "P90 upper band (veh/hr)",
        "mean":   "Mean volume (veh/hr)",
        "count":  "Records",
    }).round(2)
    ...
    with dl1:
        st.download_button(
            ...
            file_name="volume_data_with_thresholds.csv",
            mime="text/csv",
            help=(
                "Original filtered rows with p10_threshold_veh_hr, "
                "p90_upper_band_veh_hr, hourly_mean_veh_hr and below_p10_threshold "
                "columns appended — one row per volume observation."
            ),
        )
    with dl2:
        st.download_button(
            ...
            file_name="hourly_volume_thresholds.csv",
            mime="text/csv",
